...
def grafico():
    window9 = tk.Tk()
    window9.geometry("650x450")
    window9.title("Trabalho RAD")
    window9.resizable(False, False)

    conexao = sqlite3.connect("RadCadastro.db")
    cur = conexao.cursor()

    notas = cur.execute('SELECT nota FROM Aluno')
    notasTratadas = list(map(lambda lista: int(lista[0]), notas.fetchall()))

    alunos = cur.execute('SELECT nomeAluno FROM Aluno')
    alunosTratados = list(map(lambda lista: str(lista[0]), alunos.fetchall()))
    ...
    fig, ax = plt.subplots()
    ax.scatter(df['alunos'], df['notas'])
    ax.set_title('Gráfico de Dispersão')
    ax.set_xlabel('Eixo X')
    ax.set_ylabel('Eixo Y')

    canvas = FigureCanvasTkAgg(fig, master=window9)
    canvas.draw()
...
botao7 = tk.Button(window, text="Gráfico", width=20, height=2, command=grafico)
botao7.pack(pady=10)
